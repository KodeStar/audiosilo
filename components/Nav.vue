<template>
  <div :class="{ '-translate-x-full': !menu, 'translate-x-0': menu }" class="transition-all z-40 bg-gray-200 flex-shrink-0 flex-grow-0 w-screen transform lg:translate-x-0 lg:w-80 h-(screen-80) lg:h-(screen-96) border-r border-gray-300 absolute inset-0 lg:relative">
    <div v-if="collections.length > 1" class="relative border-t border-gray-100 appearance-none flex-grow collection-container">
      <select :value="currentCollection" @change="changeCollection" class="w-full py-1 px-6 bg-gray-200 text-sm leading-normal w-full">
        <option v-for="(option,index) in collections" :key="index" v-bind:value="index">
          {{ option }}
        </option>
      </select>
    </div>
    <div class="lg:border-t border-gray-100 p-8 px-6">
      ...
      <div class="flex cursor-pointer absolute bottom-0 left-0 border-t border-gray-300 w-full items-center p-5 px-10 logout" @click="logout">
